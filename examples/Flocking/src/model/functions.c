...
{
#ifdef FLOCKING_PLOT
	int separate, align, alone;
	separate = count_turtle_default_colour_variable(FLAME_GPU_VISUALISATION_COLOUR_CYAN);
	align = count_turtle_default_colour_variable(FLAME_GPU_VISUALISATION_COLOUR_YELLOW);
	alone = count_turtle_default_colour_variable(FLAME_GPU_VISUALISATION_COLOUR_MAGENTA);

	outputFile << getIterationNumber() << " " << separate << " " << align << " " << alone << std::endl;
#endif // FLOCKING_PLOT
}
...
__FLAME_GPU_INIT_FUNC__ void setup()
{
	...
	else outputFile << "# itno alignment separation alone" << std::endl;
#endif // FLOCKING_PLOT
}
